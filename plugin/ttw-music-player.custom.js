...
var _TTW_PLAYER_LAYOUT;
var _TTW_PLAYER_MGR;

jQuery.get(dmck_audioplayer.plugin_url + '/plugin/ttw-music-player-layout.php', function( data ) {
	_TTW_PLAYER_LAYOUT = data;
}, 'html');  // or 'text', 'xml', 'more'
jQuery.get(dmck_audioplayer.plugin_url + '/plugin/ttw-music-player-mgr.php', function( data ) {
	_TTW_PLAYER_MGR = data;
}, 'html');  // or 'text', 'xml', 'more'
        ...
        playlistMgr = function() {
            ...
            function buildPlaylist() {
                ...
                for (var j = 0; j < myPlaylist.length; j++) {
                    ...
                    $track.find(cssSelector.excerpt).html(set_excerpt(j));
                    
                    setRating('track', $track, j);

                    setBuyLink($track, j);
                    
                    set_permalink($track, j);
                    
                    set_tags($track, j);
                    
                    set_playlist_thumb($track, j);
                ...
                $tracks.find('.title, .excerpt, .playlist_thumb').click(function() {
            ...
            function set_permalink($track, index) {
                if (!isUndefined(myPlaylist[index].permalink)) {
                    $track.find(cssSelector.permalink).removeClass(attr(cssSelector.buyNotActive)).attr('href', myPlaylist[index].permalink).html( myPlaylist[index].moreinfo );
                }
            }
            function set_tags($track, index) {
                if (!isUndefined(myPlaylist[index].tags)) {
                    $track.find(cssSelector.tags).html(myPlaylist[index].tags);
                }
            }            
            function set_playlist_thumb($track, index) {
                if (!isUndefined(myPlaylist[index].playlist_thumb)) {
                	var img = $('<img>');
                	
                	img.attr('src', myPlaylist[index].playlist_thumb);
                	img.attr('width', 32);
                    img.attr('height', 32);
                    
                    $track.find(cssSelector.playlistThumb).append(img);
                }
            }            
...
//                  });                        
        ...
        function set_excerpt(index) {
            if (!isUndefined(myPlaylist[index].excerpt))
            	return myPlaylist[index].excerpt;
            else return '';
        }
