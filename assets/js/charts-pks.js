...
//TODO: upgrade chart.js to latest stable.
//TODO: chart color variation script
//TODO: option to disable chart colors and use DOM defaults.
...
window._dmck_charts_pkg = {
    top_requests_chart: function (selector) {
         ...
         */
        if (typeof top_10_json === 'undefined') {
            return;
        }
        if (!top_10_json.data.length) {
            return;
        }
    ...
    post_chart: function () {
        ...
        for (let x in post_chart_json) {
            if (typeof chart_data[post_chart_json[x].target] === "undefined") {
                chart_data[post_chart_json[x].target] = {
                    labels: [],
                    data: [],
                    filename: ""
                }
